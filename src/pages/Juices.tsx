  ...
  return (
    <div className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {juices.map((juice) => (
          <div key={juice.name} className="relative flex items-end bg-white rounded-xl overflow-hidden shadow-md border border-[var(--coffee-accent)]/20 card-hover min-h-[260px]">
            <img src={juice.src} alt={juice.name} className="absolute inset-0 w-full h-full object-cover" />
            ...
            <div className="relative p-6 pt-20 w-full text-white">
              <h3 className="text-xl font-bold mb-2">{juice.name}</h3>
              <p className="text-white/90">{juice.description}</p>
            </div>
          </div>
        ))}
      </div>
